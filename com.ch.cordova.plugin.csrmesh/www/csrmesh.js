...
module.exports = {
		connect : function(address,success,error){  //参数：android端为address地址，ios端为startscan扫描广播来的uuid
//			alert("findMesh");
...
//						"csrmesh","setNextDeviceId",[window.mesh1.nextDeviceId]);// mesh.nextDeviceId    //2017.6.10注释，setNextDeviceId方法重复
				success(aa);
				
			};
			exec(successWraper,error,"csrmesh","connect",[address]);
...
//			var colorToNative = color.rgb;
		...
		setBrightness:function(deviceId,bright,success,error){
			//@params : bright  [0-100)
			//			success()
			console.log("setBrightness ----- csrmesh.js");
		...
		setPower : function(deviceId,state,success,error){
			console.log("setPower ---- csrmesh.js");
			...
			try{
				if(deviceId>300){
					...
					window.mesh1.getGroup(deviceId).color = color;//属性直接使用，不甚合理    //2017.6.13 分组setPower报错，原因：can't find variable:color.故注释
				}
...
//		connect:function(devicei_uuid, success,error){ // 2017.6.9 ,for ios platform
//			console.log("csrmesh.js---connect");
//			exec(success,error, 'csrmesh', 'connect', [devicei_uuid]);
//		},
		getState:function(){
...
//			//封装 success 中 增加处理：新增灯泡并默认赋名：light+n
			...
			exec(success,error,"csrmesh","associate",[uuidHash]);
		...
		setGroups:function(deviceId,groupIds,success,error){
			...
			console.log("setGroups ----- csrmesh.js");
			exec(success,error,"csrmesh","setGroups",[deviceId,groupIds]); // 2017.6.14  groupIds.toString()修改为groupIds ;for iOS platform
			// 2017年3月31日 
			//应该将 success 包装起来，将  对mesh 的操作封装 --- 待后续实现
			try{
...
//				}
//2017-5-18 ：与IOS系统兼容
			console.log("getAssociableDevice --- csrmesh.js");
			var successWraper = function(message){
				console.log("getAssociableDevice --- csrmesh.js,  message: " + message);
				var device = {};
				if(typeof message === "string"){//IOS传过来的参数
					...
					success(message);  // 2017.6.10 加，直接传回来自native端的UUID字符串
				}else{//JSON对象为Android传过来的参数
					device = message;
					success(device);
				}
//				success(device);   // 2017.6.10
			}
		...
		resetDevice:function(deviceId, success, error){   
			console.log("resetDevice -- csrmesh.js");
			exec(success, error,"csrmesh","resetDevice",[deviceId]);	// 2017.6.10加，for ios platform
		},
