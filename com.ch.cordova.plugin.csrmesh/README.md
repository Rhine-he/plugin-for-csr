 ...
 * Scan for peripherals  		 //扫描外设
 * Connect to a peripheral		 // 连接外设
 * Read the value of a characteristic		// 读取特征值的值 
 * Write new value to a characteristic		// 往特征值写值
 * Get notified when characteristic's value changes		// 当特征值改变时收到通知

Advertising information is returned when scanning for peripherals.   // 扫描外设时返回广播信息
Service, characteristic, and property info is returned when connecting to a peripheral.	  // 连接外设时返回service等信息
All access is via service and characteristic UUIDs. The plugin manages handles internally.
// 所有的访问是通过服务的UUID和特征的UUID。 插件内部管理句柄，
// 支持与多个外设连接
...
### Description
// scan函数用来扫描低功耗蓝牙设备。每次发现外设调用成功回调。特定秒数之后扫描自动停止。
